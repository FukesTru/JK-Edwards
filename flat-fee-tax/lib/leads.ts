 ...
 *   CONTACT_TO_EMAIL        …to this inbox (the address in site.config.ts, for example)
 *   CONTACT_FROM_EMAIL      …from a verified sender, e.g. "Website <web@yourdomain.com>"
 */
...
const FORM_LABELS: Record<string, string> = {
  "get-started": "New client request",
  quick: "Website quick request",
...
async function sendWebhook(url: string, lead: Lead) {
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ source: site.brandName, summary: summarize(lead), ...lead }),
