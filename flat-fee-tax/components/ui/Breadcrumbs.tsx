...
/**
 * Breadcrumb bar rendered directly below each inner-page hero, plus
 * BreadcrumbList structured data. "Home" is prepended automatically.
 */
export function Breadcrumbs({ items }: { items: Crumb[] }) {
  const crumbs: Crumb[] = [{ name: "Home", path: "/" }, ...items];
  return (
    <div className="surface-light border-b border-line bg-white">
      <Container>
                ...
                <li key={crumb.path} className="inline-flex items-center gap-1.5">
                  {isLast ? (
                    ...
                    <>
                      <Link
                        href={crumb.path}
                        className="inline-block py-1 underline-offset-2 hover:text-navy hover:underline"
                      >
                        {crumb.name}
                      </Link>
                      <ChevronRight aria-hidden className="h-3.5 w-3.5 text-gold" strokeWidth={1.5} />
                    </>
                  )}
                </li>
              );
