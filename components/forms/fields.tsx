...
const controlClasses: Record<FieldTone, string> = {
  light:
    "border-line bg-white text-ink placeholder:text-charcoal/85 focus:border-accent focus:ring-accent/20 hover:border-charcoal/40",
  dark: "border-white/15 bg-white/[0.06] text-white placeholder:text-mist focus:border-accent-light focus:ring-accent-light/25 hover:border-white/30",
};
...
function Label({
  ...
  return (
    ...
    >
      {children}
      {required ? (
        <span aria-hidden className={tone === "dark" ? "text-accent-light" : "text-accent-strong"}>
          {" "}
          *
        </span>
      ) : (
        <span className={cn("ml-1 text-xs font-normal", tone === "dark" ? "text-mist" : "text-charcoal")}>
          (optional)
        </span>
      )}
    </label>
...
export function TextField({
  ...
  return (
    <div className={className}>
      ...
      />
      {hint && (
        <p id={`${fieldId}-hint`} className={cn("mt-1.5 text-xs", tone === "dark" ? "text-mist" : "text-charcoal")}>
          {hint}
        </p>
      )}
    </div>
