...
export function SiteHeader({ nav, phone, payBillHref }: Props) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        inert={!mobileOpen}
        className={cn(
          "fixed inset-0 z-[60] flex flex-col bg-ink text-white transition-[transform,visibility] duration-300 ease-out lg:hidden",
      ...
      >
                ...
                <li key={entry.id}>
                  ...
                  >
                    ...
                    <ChevronDown
                      aria-hidden
                      className={cn(
                        "h-5 w-5 text-accent-light transition-transform duration-300",
                  ...
                  <div id={`mobile-section-${entry.id}`} hidden={mobileSection !== entry.id} className="pb-4">
                    ...
                    {entry.footer && (
                      <Link
                        ...
                        onClick={() => setMobileOpen(false)}
                        className="mt-1 inline-flex items-center gap-2 px-1 text-sm font-semibold text-accent-light"
                      >
                        {entry.footer.label}
        ...
        <div className="grid shrink-0 grid-cols-2 gap-3 border-t border-white/10 px-5 pt-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:px-6">
          <a href={phone.href} className={buttonClasses("outline-light", "md", "px-3")}>
            <Phone aria-hidden className="h-4 w-4" />
            Call
          </a>
          <Link href="/contact" onClick={() => setMobileOpen(false)} className={buttonClasses("primary", "md", "px-3")}>
            Book
            <ArrowRight aria-hidden className="h-4 w-4 transition-transform group-hover:translate-x-1" />
          </Link>
        </div>
  ...
  return (
    ...
    >
      <button
        ...
        className={cn(
          "inline-flex items-center gap-1 rounded-md px-3 py-2 text-[15px] font-medium transition-colors hover:text-white",
      ...
      >
        ...
        <ChevronDown
          aria-hidden
          className={cn("h-4 w-4 text-accent-light transition-transform duration-200", open && "rotate-180")}
          strokeWidth={1.75}
        />
      </button>
      ...
      >
        <div
          className={cn(
            "overflow-hidden rounded-2xl bg-white text-ink shadow-[0_24px_60px_-20px_rgba(22,24,27,0.45)] ring-1 ring-black/5",
            menu.variant === "mega" ? "w-[720px]" : "w-[340px]",
          )}
        >
          <div className={cn("grid gap-1 p-3", menu.variant === "mega" && "grid-cols-2 gap-4 p-4")}>
            {menu.groups.map((group, index) => (
              <div key={group.title ?? index}>
                {group.title && (
                  <p className="px-3 pt-1 pb-2 text-[11px] font-semibold tracking-[0.16em] text-charcoal uppercase">
                    {group.title}
                  </p>
                )}
                <ul className="space-y-0.5">
                  {group.items.map((item) => (
                    <li key={item.href}>
                      ...
                      >
                        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-accent/10 text-accent transition-colors group-hover/item:bg-accent group-hover/item:text-white">
                          {item.icon}
                        </span>
                        <span>
                          <span className="block text-[15px] font-semibold text-ink">{item.label}</span>
                          {item.description && (
                            <span className="mt-0.5 block text-[13px] leading-snug text-charcoal">
                              {item.description}
                            </span>
                          )}
                        </span>
          ...
          {menu.footer && (
            <div className="flex items-center justify-between gap-4 border-t border-line bg-paper px-6 py-4">
              {menu.footer.note && <p className="text-[13px] leading-snug text-charcoal">{menu.footer.note}</p>}
              <Link
                href={menu.footer.href}
                className="group inline-flex shrink-0 items-center gap-1.5 text-sm font-semibold text-accent-strong hover:text-accent"
              >
                {menu.footer.label}
                <ArrowRight aria-hidden className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Link>
