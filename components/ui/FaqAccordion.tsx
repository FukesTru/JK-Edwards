        ...
        {items.map((item) => (
          <details key={item.q} className="faq-item group">
            <summary className="flex items-start justify-between gap-6 px-5 py-5 text-left transition-colors hover:bg-paper/70 sm:px-7 sm:py-6">
              <span className="text-[1.05rem] leading-snug font-semibold text-ink group-open:text-accent-strong">
                {item.q}
              </span>
              <span
                aria-hidden
                className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full border border-line text-accent transition-transform duration-300 group-open:rotate-45 group-open:border-accent/40 group-open:bg-accent/5"
              >
                <Plus className="h-4 w-4" strokeWidth={1.75} />
              </span>
            </summary>
            <div className="px-5 pb-6 text-[1.02rem] leading-relaxed text-charcoal sm:px-7">
