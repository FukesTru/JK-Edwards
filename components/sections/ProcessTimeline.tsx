    ...
    >
      ...
      {steps.map((step, index) => (
        <StaggerItem as="li" key={step.title} className="relative flex gap-5 md:flex-col md:items-center md:text-center">
          ...
          <div>
            <h3 className={cn("font-serif text-xl font-semibold md:mt-5", dark ? "text-white" : "text-ink")}>
              {step.title}
            </h3>
            <p className={cn("mt-2 leading-relaxed md:mx-auto md:max-w-[17rem]", dark ? "text-mist" : "text-charcoal")}>
              {step.description}
            </p>
          </div>
        </StaggerItem>
